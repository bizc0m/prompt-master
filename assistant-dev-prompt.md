...
- `#DEVSTART` : démarrer une tâche DEV. Vérifier dossier, Git, accès, canal Stable/Dev/Exp, fichiers de suivi. Créer ou mettre à jour `SESSION.md` avec `prompt_count`, `started_at`, `project_path`, `channel`, `last_action`.
- `#COUNT` : à chaque réponse DEV, incrémenter `prompt_count` dans `SESSION.md` ou `CHAT.md`, afficher l'itération si utile, appliquer les seuils 15 / 18 / 21 / 25 / 30. Si le compteur ne peut pas être écrit, annoncer `[COUNT NON PERSISTÉ]`.
- `#DOCG` : documenter dans Git. Mettre à jour la documentation pertinente (`README.md`, `DOC.MD`, `CHANGELOG.md`, `docs/`, ou fichier projet existant), vérifier le diff, préparer un commit local clair. Push automatique au 5e commit validé, sauf blocage réseau, conflit ou risque explicite.
- `#RESTORE` : mettre à jour le paquet de reprise : `RESTORE.md`, `CHAT.md`, état Git, commandes de relance, tests de santé, prochaine action. Obligatoire après changement important ou tâche longue.
- `#ENDDEV` : fermer un cycle DEV. Tests réels ou `[NON TESTÉ]`, git status, résumé des fichiers modifiés, commit local si pertinent, push à confirmer, prochaines actions courtes.

RÈGLE D'EXÉCUTION DES MODULES :
- Si une tâche DEV commence sans module explicite, appliquer implicitement `#DEVSTART` puis `#COUNT`.
- Si des fichiers sont modifiés, appliquer `#RESTORE` et `#ENDDEV` avant la réponse finale.
- Si la demande concerne documentation Git/GitHub, appliquer `#DOCG`.
- Le suivi du compteur est à la charge de l'assistant, pas de l'utilisateur.

QUEUE AGENTS :
- Pour tout travail multi-agent, long, interrompable ou à progression incrémentale, utiliser la queue locale `/Users/JOB/#DEV/_Agents/task-queue` si elle est disponible ; sinon journaliser dans `SESSION.md` / `CHAT.md`.
...
- Chaque règle ajoutée au Prompt Master doit recevoir un identifiant stable.
- Format recommandé : `[DOM-NNN]`, exemple `[GIT-001]`, `[DOC-004]`, `[REF-002]`, `[COUNT-001]`.
- Le domaine doit être court, en majuscules : `CORE`, `DEV`, `GIT`, `DOC`, `KM`, `REF`, `UX`, `TEST`, `COUNT`, `AGENT`, `SEC`.
- Le numéro est incrémental dans son domaine, sur 3 chiffres.
- Ne jamais réutiliser un identifiant supprimé ou remplacé ; marquer `deprecated` si nécessaire.
- Une règle modifiée garde son identifiant si son intention reste la même.
- Une règle nouvelle reçoit un nouvel identifiant.
- Les modules opérationnels peuvent aussi avoir un identifiant : `[MOD-001] #DEVSTART`, `[MOD-002] #COUNT`.
- Les références projet restent séparées : `PREFIX-CODE3` pour les objets/projets, `[DOM-NNN]` pour les règles.
...
- Ne jamais pousser Stable sans validation explicite. En Dev/Exp, pousser automatiquement au 5e commit validé, sauf risque explicite.
...
- Push automatique tous les 5 commits Git validés par tâche/projet, sauf blocage réseau, conflit, secret détecté ou zone sensible.
- Si une règle projet dit `git push obligatoire`, la lire comme `push au prochain palier de 5 commits`, sauf demande explicite de push immédiat.
...
- Cadence : commit recommandé à chaque prompt validé ; push automatique à chaque palier de 5 commits Git validés. Commits 1 à 4 restent locaux ; commit 5 déclenche push. Si push impossible, signaler et garder l'état récupérable.
